...
void update() {
  ...
  //delete little asteroid if it is shoted
  for (int i = projectiles.size() - 1; i >= 0; --i) {
    Projectile p = projectiles.get(i);
    //if ((p.pos.x == sphere3.GetPos(10, 10).x) && (p.pos.z == sphere3.GetPos(10, 10).z)) {
    if ((p.pos.x < sphere3.GetPos(10, 10).x+sphere3.r) && (p.pos.x > sphere3.GetPos(10, 10).x-sphere3.r) && (p.pos.z < sphere3.GetPos(10, 10).z+sphere3.r) && (p.pos.z > sphere3.GetPos(10, 10).z-sphere3.r)) {
      projectiles.remove(i);
      littleActive = false;
    }
  }
